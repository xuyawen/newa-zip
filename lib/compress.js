...
async function compress(filePath) {
	const spinner = ora(`[${chalk.yellowBright(filePath)}] is compressing...`).start();
	try {
		...
		console.log(`\n${figures.tick} Completed: Old Size ${oldSize}, New Size ${newSize}, Optimization Ratio ${ratio}`);
	} catch (err) {
		console.log(`\n${figures.cross} Compress is failed: ${chalk.redBright(err)}`)
	} finally {
		return spinner;
	}
}
